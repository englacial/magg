...
schedule ([Pyramid levels](#pyramid-levels-zagg-pyramid2), issue #382); an
...
**`levels`** — ordered `{node, cells}` entries, where `node` is the hive
...
output:
  pyramid:
    levels:                          # replaces orders/spacing WHOLESALE
...
A config that spells `levels`
([Pyramid levels](#pyramid-levels-zagg-pyramid2)) retrofits the
