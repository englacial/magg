...
leaf's own node prefix**. A column exists exactly when the store's pyramid
declaration is `zagg-pyramid/2` (§4.5) — the expanded `overviews` list always
places the declared resolutions at the shard node — and it is written by the
same worker invocation that commits the leaf, after the leaf's own stamp.
...
- **Naming.** The leaf's D23 window stem with a `.pyramid` marker:
  `{window}.pyramid.zarr`, and `all.pyramid.zarr` for the unwindowed /
  `schedule: none` leaf (the reserved token, §4.2). The `.pyramid.zarr`
  suffix is the one name seam — every leaf and overview basename ends at
  `{...}.zarr` without it, so name-grammar consumers (e.g. the root-MOC
  walker) can classify a column without opening it; classification for
  everything else is the attrs below.
